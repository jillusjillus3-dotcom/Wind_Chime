...
export function initWindCursor({ engine, render, windBodies = [], chimeRectangles = [], onUnlockAudio }) {
  ...
  const onBeforeUpdate = () => {
    ...
    activeGusts.forEach((gust, bodyId) => {
      ...
      // Apply force slightly above and off-center from body center for natural tilting and visible collision sways
      const forceOffset = (gust.body.id % 2 === 0 ? 5 : -5);
      const forceApplicationPoint = {
        x: gust.body.position.x + forceOffset,
        y: gust.body.position.y - 8
      };
