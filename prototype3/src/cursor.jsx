...
export function initWindCursor({ engine, render, windBodies = [], chimeRectangles = [], onUnlockAudio }) {
  ...
  // Render visual wind field zone rings (70px, 140px, 220px)
  const onAfterRender = () => {
    if (!render || !render.context || !isPointerOnScreen) return;

    const ctx = render.context;
    ctx.save();

    // Outer Wind Field Ring (220px - 25% Zone)
    ctx.beginPath();
    ctx.arc(mousePos.x, mousePos.y, 220, 0, 2 * Math.PI);
    ctx.strokeStyle = "rgba(56, 189, 248, 0.05)";
    ctx.lineWidth = 1;
    ctx.stroke();

    // Middle Wind Field Ring (140px - 60% Zone)
    ctx.beginPath();
    ctx.arc(mousePos.x, mousePos.y, 140, 0, 2 * Math.PI);
    ...
    // Inner Core Ring (70px - 100% Zone)
    ctx.beginPath();
    ctx.arc(mousePos.x, mousePos.y, 70, 0, 2 * Math.PI);
    ctx.strokeStyle = "rgba(251, 191, 36, 0.22)";
    ctx.lineWidth = 1.5;
    ctx.stroke();

    // Center Wind Source Glow
    const gradient = ctx.createRadialGradient(
      mousePos.x, mousePos.y, 0,
      mousePos.x, mousePos.y, 18
    );
    ...
    ctx.beginPath();
    ctx.arc(mousePos.x, mousePos.y, 18, 0, 2 * Math.PI);
