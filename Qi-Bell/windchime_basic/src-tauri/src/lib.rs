...
static IS_WOOD_SKIN: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
...
fn save_position_to_disk(app: &AppHandle, x: i32, y: i32) {
    ...
    let is_wood = IS_WOOD_SKIN.load(std::sync::atomic::Ordering::Relaxed);
    let skin = if is_wood { "wood" } else { "basic" }.to_string();
    if let Some(path) = get_settings_path(app) {
        let settings = WindowSettings { x, y, locked, muted, skin };
    ...
    if path.exists() {
        if let Ok(content) = fs::read_to_string(path) {
            if let Ok(settings) = serde_json::from_str::<WindowSettings>(&content) {
                ...
                let is_wood = settings.skin.to_lowercase() == "wood";
                IS_WOOD_SKIN.store(is_wood, std::sync::atomic::Ordering::SeqCst);
...
#[tauri::command]
fn get_active_skin() -> String {
    if IS_WOOD_SKIN.load(std::sync::atomic::Ordering::Relaxed) {
        "wood".to_string()
    } else {
        "basic".to_string()
    }
...
#[tauri::command]
fn set_active_skin(app: AppHandle, skin: String) -> Result<String, String> {
    let is_wood = skin.to_lowercase() == "wood";
    IS_WOOD_SKIN.store(is_wood, std::sync::atomic::Ordering::SeqCst);
    let new_skin = if is_wood { "wood" } else { "basic" }.to_string();
    if let Some(w) = app.get_webview_window("main") {
        let _ = w.emit("skin-changed", &new_skin);
        if let Ok(pos) = get_window_position(w) {
            save_position_to_disk(&app, pos.0, pos.1);
        }
    }
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let skin_basic_item = MenuItem::with_id(app, "skin_basic", "Basic Chime (Aluminum)", true, None::<&str>)?;
            let skin_wood_item = MenuItem::with_id(app, "skin_wood", "Wood Chime (Bamboo)", true, None::<&str>)?;
            let change_chime_menu = Submenu::with_items(app, "Change Chime", true, &[&skin_basic_item, &skin_wood_item])?;

            let uninstall_item = MenuItem::with_id(app, "uninstall", "Uninstall", true, None::<&str>)?;

            let tray_menu = Menu::with_items(
                app,
                &[&lock_item, &mute_item, &change_chime_menu, &uninstall_item],
            )?;
            ...
            TrayIconBuilder::new()
                ...
                .on_menu_event(move |app, event| match event.id.as_ref() {
                    "skin_basic" => {
                        IS_WOOD_SKIN.store(false, std::sync::atomic::Ordering::SeqCst);
                        if let Some(window) = app.get_webview_window("main") {
                            let _ = window.emit("skin-changed", "basic");
                    ...
                    "skin_wood" => {
                        IS_WOOD_SKIN.store(true, std::sync::atomic::Ordering::SeqCst);
                        if let Some(window) = app.get_webview_window("main") {
                            let _ = window.emit("skin-changed", "wood");
                            if let Ok(pos) = get_window_position(window.clone()) {
                                save_position_to_disk(app, pos.0, pos.1);
                            }
                        }
